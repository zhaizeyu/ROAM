...
TEST_USER_NAME=roam-test
TEST_USER_DISPLAY_NAME=ROAM 测试用户
TEST_USER_PASSWORD=请设置随机强密码
NODE_ENV=production
```

`OPENAI_API_KEY`、`DATABASE_URL` 和 `TEST_USER_PASSWORD` 只需勾选 **Runtime Variable**，关闭 **Build Variable**，避免密钥进入镜像构建信息。无需在 Coolify 环境变量中配置 `PORT` 或 `HOSTNAME`，也无需把容器端口映射到宿主机。程序首次访问数据库时会自动创建独立的 `roam` schema，不会改动 `public` schema 中的其他项目表。
...
- `users` 和 `auth_sessions` 保存注册账号、密码哈希和可撤销的登录会话；明文密码不写入数据库。

“历史行程”按登录用户隔离，同一账号换浏览器仍能继续查看。永久链接额外使用随机访问令牌；拿到完整链接的人在登录后也能打开该行程，因此分享链接相当于授予行程编辑访问权。

测试模式会自动预置 `TEST_USER_NAME` 对应的测试用户，登录页点击“确定并进入”即可创建会话，无需把测试密码发送到浏览器。所有使用快捷入口的人共享同一个测试账号和历史行程。正式开放前必须设置 `ENABLE_TEST_LOGIN=false`，并保留普通登录/注册流程。
...
- 生产化前建议增加邮箱验证/找回密码、数据库备份与保留策略、按用户限流、用量计费和人工反馈。
