...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!uuidPattern.test(id) || !user && ![auth.accessToken, auth.clientId].some((value) => value && uuidPattern.test(value))) {
      return NextResponse.json({ error: "行程链接无效。" }, { status: 400, headers: noStore });
    }
    const trip = await getTrip(id, auth.accessToken, auth.clientId, user?.id);
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const user = await getAuthenticatedUser(request);
    if (!uuidPattern.test(id) || !user && ![accessToken, clientId].some((value) => value && uuidPattern.test(value))) {
    ...
    await logEvent({ event: "trip_manual_edit", message: "用户保存了手工行程修改", tripId: id, userId: user?.id, metadata: { version: updated.current_version } });
